...
namespace SmartMonitoring.Framework.Models.SmartTrack
{
    ...
    public class GeoCoordinate : IEquatable<GeoCoordinate>
    {
        ...
        // Additional Method to detech GeoFence
        public bool IsWithinPolygon(List<GeoCoordinate> points)
        {
            bool inside = false;
            GeoCoordinate p = this;
            // An imaginary closing segment is implied,
            // so begin testing with that.
            GeoCoordinate v1 = points[points.Count - 1];

            foreach (GeoCoordinate v0 in points)
            {
                double d1 = (p.Latitude - v0.Latitude) * (v1.Longitude - v0.Longitude);
                double d2 = (p.Longitude - v0.Longitude) * (v1.Latitude - v0.Latitude);

                if (p.Latitude < v1.Latitude)
                {
                    // V1 below ray
                    if (v0.Latitude <= p.Latitude)
                    {
                        // V0 on or above ray
                ...
                else if (p.Latitude < v0.Latitude)
                {
